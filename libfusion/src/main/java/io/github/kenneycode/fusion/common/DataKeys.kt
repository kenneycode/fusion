package io.github.kenneycode.fusion.common

class DataKeys {

    companion object {

        const val ST_MATRIX = "ST_MATRIX"
        const val KEY_DISPLAY_WIDTH = "KEY_DISPLAY_WIDTH"
        const val KEY_DISPLAY_HEIGHT = "KEY_DISPLAY_HEIGHT"
        const val KEY_SOURCE_VIDEO_FPS = "KEY_SOURCE_VIDEO_FPS"
        const val KEY_SOURCE_VIDEO_WIDTH = "KEY_SOURCE_VIDEO_WIDTH"
        const val KEY_SOURCE_VIDEO_HEIGHT = "KEY_SOURCE_VIDEO_HEIGHT"

    }

}